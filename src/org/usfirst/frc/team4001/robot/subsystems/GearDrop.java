...
public class GearDrop extends Subsystem {
    ...
    public boolean gearIsInside() {
    	if (IRSensor.getAverageValue() > NumberConstants.IR_sensor_treshold) {
    		return true;
    	} else {
    		return false;
    	}
    ...
    public void open(){
    	...
    	if(this.rightswitchpressed()){
    		rightPower = 0.0;
    	}
    	if(this.leftswitchpressed()){
    		leftPower = 0.0;
    	}
    	this.openRightHolder(rightPower);
    	this.openLeftHolder(leftPower);
    }
